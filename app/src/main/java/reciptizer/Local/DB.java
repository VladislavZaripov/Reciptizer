...
public class DB {
    ...
    public void open() {
        ...
        Log.d(Activity_Main.LOG_TAG,"DB_open");
    }

    public void close() {
        if (dbHelper != null) dbHelper.close();
        Log.d(Activity_Main.LOG_TAG,"DB_close");
    }
    ...
    public static RecipeFilter getRecipeFilterResult (String recipeFilterName, String category, String kitchen, String preferences) {
        Log.d(Activity_Main.LOG_TAG,"recipe = " + recipeFilterName + "; category = " + category + "; kitchen = " + kitchen + "; preferences = " + preferences);
        ...
        if (cursor.moveToFirst()) {
            do {
                int id = cursor.getInt(cursor.getColumnIndex(DB.TABLE1_COLUMN_ID));
                String recipe = cursor.getString(cursor.getColumnIndex(DB.TABLE1_COLUMN_RECIPE));
                String img_title = cursor.getString(cursor.getColumnIndex(DB.TABLE1_COLUMN_IMG_TITLE));
                int time = cursor.getInt(cursor.getColumnIndex(DB.TABLE1_COLUMN_TIME));
    ...
    private static Table1 getRecipeTable1 (int idRecipe) {
        ...
        Integer time = cursor.getInt(cursor.getColumnIndex(DB.TABLE1_COLUMN_TIME));
        Integer portion = cursor.getInt(cursor.getColumnIndex(DB.TABLE1_COLUMN_PORTION));
        String img_full = cursor.getString(cursor.getColumnIndex(DB.TABLE1_COLUMN_IMG_FULL));
        String img_title = cursor.getString(cursor.getColumnIndex(DB.TABLE1_COLUMN_IMG_TITLE));
        cursor.close();
    ...
    private static List<Table2Row> getRecipeTable2 (int idRecipe) {
        ...
        if (cursor.moveToFirst()) {
            do {
                Integer id = cursor.getInt(cursor.getColumnIndex(DB.TABLE2_COLUMN_ID));
                String ingredients = cursor.getString(cursor.getColumnIndex(DB.TABLE2_COLUMN_INGREDIENTS));
                Integer quantity = cursor.getInt(cursor.getColumnIndex(DB.TABLE2_COLUMN_QUANTITY));
    ...
    private static List<Table3Row> getRecipeTable3 (int idRecipe) {
        ...
        if (cursor.moveToFirst()) {
            do {
                Integer id = cursor.getInt(cursor.getColumnIndex(DB.TABLE3_COLUMN_ID));
                Integer number = cursor.getInt(cursor.getColumnIndex(DB.TABLE3_COLUMN_NUMBER));
                String text = cursor.getString(cursor.getColumnIndex(DB.TABLE3_COLUMN_TEXT));
                String img_full = cursor.getString(cursor.getColumnIndex(DB.TABLE3_COLUMN_IMG_FULL));
                String img_title = cursor.getString(cursor.getColumnIndex(DB.TABLE3_COLUMN_IMG_TITLE));
    ...
    public static void addRecipeFromServer (final Recipe recipe, Activity activity){
        ...
        Log.d(Activity_Main.LOG_TAG, "addRecipeFromServer: recipe.table1.TABLE1_COLUMN_IMG_TITLE = " + recipe.table1.TABLE1_COLUMN_IMG_TITLE);

        if (recipe.table1.TABLE1_COLUMN_IMG_TITLE!=null && !recipe.table1.TABLE1_COLUMN_IMG_TITLE.equals("null"))
        {
            Bitmap bitmap_png = BitmapTempSingleton.getInstance().getTempBitmap().get(recipe.table1.TABLE1_COLUMN_IMG_TITLE);
            File file = ph.createFile("png");
            recipe.table1.TABLE1_COLUMN_IMG_TITLE = file.toString();
            ph.saveBitmapToFile(bitmap_png, file);
        }

        Log.d(Activity_Main.LOG_TAG, "addRecipeFromServer: recipe.table1.TABLE1_COLUMN_IMG_TITLE = " + recipe.table1.TABLE1_COLUMN_IMG_FULL);
        if (recipe.table1.TABLE1_COLUMN_IMG_FULL!=null && !recipe.table1.TABLE1_COLUMN_IMG_FULL.equals("null"))
        {
            Bitmap bitmap_png = BitmapTempSingleton.getInstance().getTempBitmap().get(recipe.table1.TABLE1_COLUMN_IMG_FULL);
            File file = ph.createFile("png");
            recipe.table1.TABLE1_COLUMN_IMG_FULL = file.toString();
            ph.saveBitmapToFile(bitmap_png, file);
        }

        for (final Table3Row table3Row : recipe.rowsTable3) {

            Log.d(Activity_Main.LOG_TAG, "addRecipeFromServer: table3Row.TABLE3_COLUMN_IMG_TITLE = " + table3Row.TABLE3_COLUMN_IMG_TITLE);
            if (table3Row.TABLE3_COLUMN_IMG_TITLE!=null && !table3Row.TABLE3_COLUMN_IMG_TITLE.equals("null"))
            {
                Bitmap bitmap_png = BitmapTempSingleton.getInstance().getTempBitmap().get(table3Row.TABLE3_COLUMN_IMG_TITLE);
                File file = ph.createFile("png");
                table3Row.TABLE3_COLUMN_IMG_TITLE = file.toString();
                ph.saveBitmapToFile(bitmap_png, file);
            }

            Log.d(Activity_Main.LOG_TAG, "addRecipeFromServer: table3Row.TABLE3_COLUMN_IMG_FULL = " + table3Row.TABLE3_COLUMN_IMG_FULL);
            if (table3Row.TABLE3_COLUMN_IMG_FULL!=null&&!table3Row.TABLE3_COLUMN_IMG_FULL.equals("null"))
            {
                Bitmap bitmap_png = BitmapTempSingleton.getInstance().getTempBitmap().get(table3Row.TABLE3_COLUMN_IMG_FULL);
                File file = ph.createFile("png");
                table3Row.TABLE3_COLUMN_IMG_FULL = file.toString();
                ph.saveBitmapToFile(bitmap_png, file);
            }
        }
            addRecipe(recipe);
    ...
    private static int addPositionInTable1 (Recipe recipe){
        ...
        cv.put(TABLE1_COLUMN_RECIPE, recipe.table1.TABLE1_COLUMN_RECIPE);
        cv.put(TABLE1_COLUMN_CATEGORY, recipe.table1.TABLE1_COLUMN_CATEGORY);
        cv.put(TABLE1_COLUMN_KITCHEN, recipe.table1.TABLE1_COLUMN_KITCHEN);
        cv.put(TABLE1_COLUMN_PREFERENCES, recipe.table1.TABLE1_COLUMN_PREFERENCES);
        cv.put(TABLE1_COLUMN_TIME, recipe.table1.TABLE1_COLUMN_TIME);
        cv.put(TABLE1_COLUMN_PORTION, recipe.table1.TABLE1_COLUMN_PORTION);
        cv.put(TABLE1_COLUMN_IMG_FULL, recipe.table1.TABLE1_COLUMN_IMG_FULL);
        cv.put(TABLE1_COLUMN_IMG_TITLE, recipe.table1.TABLE1_COLUMN_IMG_TITLE);
    ...
    private static void addPositionInTable2 (Recipe recipe, int id_recipe) {
        for(Table2Row table2Row : recipe.rowsTable2) {
            ...
            cv.put(TABLE2_COLUMN_INGREDIENTS, table2Row.TABLE2_COLUMN_INGREDIENTS);
            cv.put(TABLE2_COLUMN_QUANTITY, table2Row.TABLE2_COLUMN_QUANTITY);
            cv.put(TABLE2_COLUMN_MEASURE, table2Row.TABLE2_COLUMN_MEASURE);
    ...
    private static void addPositionInTable3 (Recipe recipe, int id_recipe) {
        for(Table3Row table3Row : recipe.rowsTable3) {
            ...
            cv.put(TABLE3_COLUMN_NUMBER, table3Row.TABLE3_COLUMN_NUMBER);
            cv.put(TABLE3_COLUMN_TEXT, table3Row.TABLE3_COLUMN_TEXT);
            cv.put(TABLE3_COLUMN_IMG_FULL, table3Row.TABLE3_COLUMN_IMG_FULL);
            cv.put(TABLE3_COLUMN_IMG_TITLE, table3Row.TABLE3_COLUMN_IMG_TITLE);
    ...
    public static void deleteRowTable2 (int id){
        ...
        Log.d(Activity_Main.LOG_TAG, "deleteRowTable2: id = " +id + ", count = " + delCount);
    }
    ...
    public static void deleteRowTable3 (int id){
        ...
        if (cursor.moveToFirst()) {
            do {
                uri = cursor.getString(cursor.getColumnIndex(DB.TABLE3_COLUMN_IMG_FULL));
                if (uri!=null) {
                    boolean result;
                    Uri i = Uri.parse(uri);
                    File file = new File(Objects.requireNonNull(i.getPath()));
                    result = file.delete();
                    Log.d(Activity_Main.LOG_TAG, "TABLE3_COLUMN_IMG_JPEG File delete: " + result);
                }
                uri = cursor.getString(cursor.getColumnIndex(DB.TABLE3_COLUMN_IMG_TITLE));
                if (uri!=null) {
                    boolean result;
                    Uri i = Uri.parse(uri);
                    File file = new File(Objects.requireNonNull(i.getPath()));
                    result = file.delete();
                    Log.d(Activity_Main.LOG_TAG, "TABLE3_COLUMN_IMG_PNG File delete: " + result);
                }
            } while (cursor.moveToNext());
        }
    ...
    public static void deletePosition (int id){
        ...
        if (cursor.moveToFirst()) {
            uri = cursor.getString(cursor.getColumnIndex(DB.TABLE1_COLUMN_IMG_FULL));
            if (uri != null) {
                boolean result;
                Uri i = Uri.parse(uri);
                File file = new File(Objects.requireNonNull(i.getPath()));
                result = file.delete();
                Log.d(Activity_Main.LOG_TAG, "TABLE1_COLUMN_IMG_JPEG File delete: " + result);
            }
            uri = cursor.getString(cursor.getColumnIndex(DB.TABLE1_COLUMN_IMG_TITLE));
            if (uri != null) {
                boolean result;
                Uri i = Uri.parse(uri);
                File file = new File(Objects.requireNonNull(i.getPath()));
                result = file.delete();
                Log.d(Activity_Main.LOG_TAG, "TABLE1_COLUMN_IMG_PNG File delete: " + result);
            }
        }
        cursor.close();
        ...
        Log.d(Activity_Main.LOG_TAG, "deletePosition Table1: id = " +id + ", count = " + delCount);
        delCount = db.delete(TABLE2_NAME, TABLE2_COLUMN_ID_RECIPE + " = " + id, null);
        Log.d(Activity_Main.LOG_TAG, "deletePosition Table2: id_recipe = " +id + ", count = " + delCount);
        delCount = db.delete(TABLE3_NAME, TABLE3_COLUMN_ID_RECIPE + " = " + id, null);
        Log.d(Activity_Main.LOG_TAG, "deletePosition Table3: id_recipe = " +id + ", count = " + delCount);
    }
    ...
    class DBHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            Log.d(Activity_Main.LOG_TAG, "TABLE1_CREATE");
            db.execSQL(TABLE2_CREATE);
            Log.d(Activity_Main.LOG_TAG, "TABLE2_CREATE");
